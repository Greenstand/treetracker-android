...
class SessionUploaderTest {
    ...
    @Test
    fun `WHEN upload called THEN creates session requests uploads bundle and marks uploaded`() = runTest {
    ...
    @Test
    fun `WHEN upload called THEN resolves deviceConfigId to UUID via DAO`() = runTest {
        val sessionEntity = SessionEntity(
            uuid = "session-uuid-2",

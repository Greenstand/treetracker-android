            ...
            when (action) {
                ...
                is IndividualMessageListAction.NavigateToSelected -> {
                    when (val msg = state.selectedMessage) {
                        is DirectMessage -> navController.navigateSafely(ChatRoute(planterInfoId = userId, otherChatIdentifier = msg.from))
                        is SurveyMessage -> navController.navigateSafely(SurveyRoute(messageId = msg.id))
                        is AnnouncementMessage -> navController.navigateSafely(AnnouncementRoute(messageId = msg.id))
                    }
                }
